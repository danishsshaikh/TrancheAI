...
def ensure_no_negative_money(record: object, fields: list[str]) -> list[ValidationIssue]:
...
def validate_project_code_unique(project: Project, existing_codes: set[str]) -> None:
...
def validate_single_active_sanction(sanction: FundingSanction, sanctions: list[FundingSanction]) -> None:
    if sanction.status.value != "approved":
        return
    active = [s for s in sanctions if s.project_id == sanction.project_id and s.status.value == "approved" and s.id != sanction.id]
    if active:
...
def validate_tranche(
    tranche: Tranche,
    project_sanctions: list[FundingSanction],
    project_revisions: list[FundingRevision],
    project_tranches: list[Tranche],
    *,
    ...
    sequence_matches = [t for t in project_tranches if t.sequence_number == tranche.sequence_number and t.id != tranche.id and t.status != TrancheStatus.CANCELLED]
    if sequence_matches:
    ...
    if tranche.payment_reference:
        duplicate_refs = [
            ...
            if t.payment_reference == tranche.payment_reference and t.id != tranche.id and t.status != TrancheStatus.CANCELLED
        ]
        if duplicate_refs:
    ...
    if tranche.status in {TrancheStatus.APPROVED, TrancheStatus.SCHEDULED, TrancheStatus.PARTIALLY_DISBURSED, TrancheStatus.DISBURSED}:
